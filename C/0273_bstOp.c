...
int lookUpHeight, foundMyNode, foundIRNode, subTreeCount,
  subTreeInOrder[1000000], levelElementCount, level[1000000];
void
insert (node * tree, node * leaf)
{
...
void
levelCreate (node * tree, int height)
{
  if (tree)
    {
      if (height == 1)
...
void
findAndGenerate (node * tree, int val)
{
  if (tree)
    {
      if (val < tree->data)
