...
insertion (int data)
{
  ...
  bf = parent_bf = root;	/* find the location for inserting the new node */
  for (current = root; current != NULL;
       ptr = current, current = current->link[res])
    {
  ...
  res = link_dir[i = 0];	/* updating the height balance after insertion */
  ...
  if (bf->bfactor == 2)
      ...
      else
	{
	  ...
	  subtree->link[0] = bf;	/* update balance factors */
      ...
      else
	{
	  ...
	  subtree->link[1] = bf;	/* update balance factors */
	  if (subtree->bfactor == -1)
	    {
	      bf->bfactor = 1;
	      temp->bfactor = 0;
...
deletion (int data)
{
  ...
  if (!current)
    return;
  index = link_dir[i - 1];
  temp = current->link[1];	/* delete the node from the AVL tree - similar to BST deletion */
      ...
      if (link_dir[i] == 0)
	{
	  ...
	  if (x->bfactor == 1)
		...
		{		/* double rotation - (SR right + SR left) */
		  ...
		  z->link[1] = y;
		  x->link[1] = z->link[0];
		  z->link[0] = x;	/* update balance factors */
		  if (z->bfactor == -1)
		    {
		      x->bfactor = 0;
		      y->bfactor = 1;
      ...
      else
	{
	  ...
	  if (x->bfactor == -1)
		...
		{		/* double rotation - (SR right + SR left) */
		  ...
		  z->link[0] = y;
		  x->link[0] = z->link[1];
		  z->link[1] = x;	/* update balance factors */
		  if (z->bfactor == -1)
		    {
		      x->bfactor = 1;
		      y->bfactor = 0;
